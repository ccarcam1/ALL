function [keep_these, fit_again_later] = my_visualize_fits_test(pre_MSD, check_these)
% end variable would be length(gaussfit)
counteristhis = 1;
start_var = 1;
end_var = length(check_these);
for test = start_var:end_var
     i = check_these(test);
%     figure(1);
%     yyaxis left
...
%     ylim([0 15]);
    ...
    elseif choice == 7
    fit_again_later(counteristhis)= i;
